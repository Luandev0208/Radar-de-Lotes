...
class Database:
    ...
    def _create_schema(self, db):
            ...
            for column in ("description", "listing_code", "phone", "whatsapp", "email", "agency", "broker", "photos"):
    ...
    def fingerprint(item: Listing) -> str:
        ...
        parts = [item.neighborhood, item.city, str(round(item.price or 0, -3)),
    ...
    def _find_duplicate(self, db, item: Listing):
        ...
        if clean_url:
            row = db.execute("SELECT id, price FROM listings WHERE fingerprint=?", ("url:" + clean_url,)).fetchone()
            if row: return row
        if item.listing_code:
            row = db.execute("SELECT id, price FROM listings WHERE listing_code=? AND source=?", (item.listing_code, item.source)).fetchone()
            if row: return row
        # Mesma área + bairro e ao menos dois sinais fortes evitam unir terrenos diferentes.
        if item.area and item.neighborhood:
            candidates = db.execute(
                "SELECT * FROM listings WHERE area BETWEEN ? AND ?", (item.area - 1, item.area + 1)
            ).fetchall()
            for row in candidates:
                if normalize(row["neighborhood"]) != normalize(item.neighborhood): continue
                evidence = 0
                if item.address and row["address"]:
                    if normalize(item.address) == normalize(row["address"]) or token_similarity(item.address, row["address"]) >= 0.6:
                        evidence += 2
                if item.dimensions and row["dimensions"] and normalize(item.dimensions) == normalize(row["dimensions"]): evidence += 1
                if item.phone and row["phone"] and normalize(item.phone) == normalize(row["phone"]): evidence += 1
                if item.price and row["price"] and abs(item.price - row["price"]) <= 1000: evidence += 1
                if normalize(item.title) and normalize(item.title) == normalize(row["title"]): evidence += 1
                if evidence >= 2: return row
        return None

    @staticmethod
    def classify(item: Listing) -> str:
        neighborhood = normalize(item.neighborhood)
        preferred = any(normalize(n) in neighborhood for n in (
            "Nacional", "Xangri-lá", "Vale das Amendoeiras", "Bom Jesus", "Arvoredo"
        ))
        price_known = item.price is not None
        area_known = item.area is not None
        price_ok = price_known and item.price <= 420_000
        area_ok = area_known and 350 <= item.area <= 380
        if preferred and price_ok and area_ok:
            return "Muito interessante"
        if not price_known or not area_known:
            return "Pode ser interessante — precisa confirmar"
        if price_ok and area_ok:
            return "Pode ser interessante"
    ...
    def _merge(old, incoming: Listing) -> Listing:
        ...
        preserve_fields = {"dimensions", "topography", "walled", "cab", "cam", "phone", "whatsapp",
                           "email", "agency", "broker", "image_url", "contact", "listing_code"}
            ...
            if name in ("price", "area"):
                ...
                for value in (current, new):
                    for line in str(value or "").splitlines():
                        if line.strip() and line.strip() not in combined: combined.append(line.strip())
                merged[name] = "\n".join(combined)
    ...
    def _register_source(db, listing_id, item: Listing, now):
        if not item.source and not item.url: return
        db.execute(
            "INSERT OR IGNORE INTO listing_sources(listing_id,source,url,listing_code,found_at) VALUES(?,?,?,?,?)",
            (listing_id, item.source or "Fonte não informada", item.url or "", item.listing_code or "", now),
        )

    def upsert(self, item: Listing) -> tuple[int, str]:
        ...
        with self.connect() as db:
            ...
            if old:
                ...
                if item.price is not None and old["price"] != item.price:
                    db.execute("INSERT INTO price_history(listing_id,price,recorded_at) VALUES(?,?,?)",
                ...
                db.execute(f"UPDATE listings SET {assignments},classification=?,updated_at=? WHERE id=?",
                           (*values.values(), self.classify(merged_item), now, old["id"]))
            ...
            cur = db.execute(
                ...
                f"VALUES({marks},?,?,?,?)",
                (*values.values(), self.classify(item), fp, now, now),
            )
    ...
    def all(self, status: str = "new"):
        with self.connect() as db:
            low_filter = " AND classification != 'Pouco compatível'" if status == "new" else ""
            return db.execute(
                "SELECT * FROM listings WHERE status=?" + low_filter + " ORDER BY "
                "CASE classification WHEN 'Muito interessante' THEN 0 WHEN 'Pode ser interessante' THEN 1 "
                "WHEN 'Pode ser interessante — precisa confirmar' THEN 2 ELSE 3 END, found_at DESC",
                (status,),
            ).fetchall()
    ...
    def price_history(self, listing_id: int):
        with self.connect() as db:
            return db.execute("SELECT price, recorded_at FROM price_history WHERE listing_id=? ORDER BY recorded_at DESC", (listing_id,)).fetchall()

    def sources(self, listing_id: int):
        with self.connect() as db:
            return db.execute("SELECT source, url, listing_code, found_at FROM listing_sources WHERE listing_id=? ORDER BY id", (listing_id,)).fetchall()
